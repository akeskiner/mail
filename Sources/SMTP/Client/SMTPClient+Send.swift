...
extension SMTPClient {
    /**
        Send an email to connection using specified credentials, or the default
        credentials if none supplied. If there are no default credentials,
        `SMTPClientError.noCredentials` will be thrown.
    */
    @discardableResult
    public func send(_ email: Email, using auth: SMTPCredentials?) throws -> (code: Int, greeting: String) {
        return try send([email], using: auth ?? defaultCredentials)
    }
...
extension Email {
    fileprivate func makeDataHeaders() -> [String: String] {
        ...
        dataHeaders["Message-Id"] = NSUUID.smtpMessageId // was just id
