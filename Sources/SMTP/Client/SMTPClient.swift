...
public final class SMTPClient<ClientStreamType: ClientStream>: ProgramStream {
    ...
    public let defaultCredentials: SMTPCredentials?

    internal let stream: Transport.Stream

    /**
         Connect the client to given SMTP Server. If `credentials` are supplied,
         then all connections will use these credentials unless overridden on
         a per-send basis.

            try SMTPClient(host: "smtp.gmail.com", port: 465, securityLayer: .tls,
                credentials: SMTPCredentials(user: "user", pass: "pass"))
    */
    public init(host: String, port: Int, securityLayer: SecurityLayer, credentials: SMTPCredentials?) throws {
        self.host = host
        self.port = port
    ...
    /**
         Connect the client to given SMTP Server. Credentials must be supplied
         on each `send()`, or `SMTPClientError.noCredentials` will be thrown.

            try SMTPClient(host: "smtp.gmail.com", port: 465, securityLayer: .tls)
    */
    public convenience init(host: String, port: Int, securityLayer: SecurityLayer) throws {
        try self.init(host: host, port: port, securityLayer: securityLayer, credentials: nil)
    }
    ...
    internal func negotiateSession(using credentials: SMTPCredentials?, handler: (SMTPClient) throws -> Void) throws -> (code: Int, greeting: String) {
        guard let credentials = credentials ?? defaultCredentials else {
            throw SMTPClientError.noCredentials
        }
